...
# We want to encourage high win rate, so we might boost the exit profit bonus?
TrainingConfig.EXIT_PROFIT_BONUS = 0.005 # Slight boost from 0.001
...
# CONFIGURATION
# =============================================================================
# Start from the Champion Model (Ep 380)
TARGET_MODEL_PATH = "models/swing_gen7_refined_ep380" 
NEW_PREFIX = "models/swing_gen9_unbound_ep"
...
# Validating "Unbound" logic: Pure Agent Exits.
LEARNING_RATE_SCALE = 0.25  # 25% of original LR to avoid destroying what it knows
EPSILON_START = 0.10        # Higher exploration than "freeze" test, let it try new things
EPSILON_MIN = 0.01
DECAY = 0.98
...
def main():
    ...
    print(f"🚀 Starting Gen9 UNBOUND Finetuning on {device}")
    ...
    action_dim = 3
    # Fix: Match the original training configuration from train_swing_phase2.py
    # where vision_channels was set to num_features (11) by mistake or design.
    # We must replicate this to load the weights correctly.
    ...
    writer = SummaryWriter(log_dir=f"logs/runs/gen9_unbound")
    ...
    for episode in range(START_EPISODE, TOTAL_EPISODES + 1):
        ...
        if episode % 5 == 0:
            ...
            # CSV Log
            csv_path = f"logs/trades_swing_gen9_unbound_ep{episode}.csv"
