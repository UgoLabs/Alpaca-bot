"""
Sweep options_bearish_from_swing checkpoints (bearish-only portfolio backtest).

BUY opens put_debit -> bear_call_credit; SELL closes held spreads.

Usage:
  .\\.venv\\Scripts\\python.exe scripts/sweep_options_bearish_from_swing.py --disk-cache --no-spy-filter
  .\\.venv\\Scripts\\python.exe scripts/sweep_options_bearish_from_swing.py --only 40,45,50,best --disk-cache
"""
...
def main():
    ap = argparse.ArgumentParser(description="Sweep bearish options checkpoints")
    ...
    ap.add_argument(
        ...
        help="BUY threshold (default BEARISH_CONFIDENCE_THRESHOLD)",
    )
    ...
    ap.add_argument(
        "--only", default="40,45,50,best",
        help="Comma-separated checkpoint labels (ep40 -> 40 or ep40)",
    )
    args = ap.parse_args()

    buy_conf = args.confidence_threshold
    if buy_conf is None:
        buy_conf = float(getattr(OptionsTraderConfig, "BEARISH_CONFIDENCE_THRESHOLD", 0.65))

    checkpoints = _filter_checkpoints(_discover_checkpoints(args.prefix), args.only)
    if not checkpoints:
        print(f"No checkpoints matched --only={args.only!r} for models/{args.prefix}_*")
        return
    ...
    print("=" * 88)
    print("BEARISH OPTIONS CHECKPOINT SWEEP")
    print(f"  Watchlist: {args.watchlist}")
    print(f"  Marks: {cache_note}")
    print(f"  Period: {args.test_start_date} -> {args.test_end_date}")
    print(f"  Bearish BUY conf > {buy_conf}, SELL close conf > {args.sell_confidence_threshold}")
    print(f"  Strategies: {getattr(OptionsTraderConfig, 'BEARISH_STRATEGIES', ())}")
    ...
    rows = []
    for label, path in tqdm(checkpoints, desc="Bearish checkpoints"):
        try:
