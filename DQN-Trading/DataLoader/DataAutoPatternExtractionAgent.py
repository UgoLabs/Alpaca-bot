...
class DataAutoPatternExtractionAgent(Data):
    ...
    def find_trend(self, window_size=20):
        ...
        # Initialize the trend_class column first
        self.data['trend_class'] = 0

        for index in range(len(self.data)):
            if index >= window_size:
                moving_average_history = self.data['MA'].iloc[index - window_size:index].values
                difference_moving_average = np.diff(moving_average_history).mean()
                # Use .loc for assignment
                self.data.loc[self.data.index[index], 'trend_class'] = 1 if difference_moving_average > 0 else 0
