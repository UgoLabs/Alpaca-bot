...
class VectorizedTradingEnv:
    ...
    def step(self, actions):
        ...
        # ===== GEN15: MOMENTUM & TREND FILTERS =====
        if self.in_position.any():
            unrealized_pnl_pct = (current_prices - self.entry_price) / torch.clamp(self.entry_price, min=0.01)
            
            # 1. UNDERWATER PENALTY (MOMENTUM RULE: CUT LOSERS FAST)
            # Momentum trades should work immediately. If red, get out.
            losing_positions = self.in_position & (unrealized_pnl_pct < -0.01) # Allow 1% wiggle 
            if losing_positions.any():
                ...
                precision_penalty = (losses ** 2) * 50.0 * hold_time_factor * self.reward_scale
                rewards[losing_positions] -= precision_penalty

            # 2. MOMENTUM HOLDING BONUS
            # Only reward holding if we are OUTPERFORMING
            winning_positions = self.in_position & (unrealized_pnl_pct > 0.01) 
            if winning_positions.any():
                 rewards[winning_positions] += 0.005 * self.reward_scale

        # ===== TREND ALIGNMENT PENALTY (Don't Fight the Trend) =====
        # Detect trend using lookback
        lookback_step = torch.clamp(self.current_step - self.trend_lookback, min=0)
        lookback_prices = self.prices[self._row_indices, lookback_step]
        
        # Bullish: Price > Lookback (Trend Up)
        # Bearish: Price < Lookback * 0.98 (Trend Down)
        is_bullish = current_prices > lookback_prices
        is_bearish = current_prices < (lookback_prices * 0.98)
        
        # PENALIZE: Buying the Dip in a Bear Trend (Falling Knife)
        # We want to kill the Ep480 behavior of buying crashes.
        knife_catchers = (effective_actions == 1) & is_bearish
        if knife_catchers.any():
            rewards[knife_catchers] -= 0.05 * self.reward_scale # Moderate penalty for fighting trend
            
        # REWARD: Buying Breakouts
        # Buying when price is higher than lookback (Momentum)
        breakout_buyers = (effective_actions == 1) & is_bullish
        if breakout_buyers.any():
             rewards[breakout_buyers] += 0.01 * self.reward_scale
